...
def format_page_override_md(design_system: dict, page_name: str, page_query: str = None) -> str:
    ...
    lines.append("")
    notes = page_overrides.get("notes", [])
    if notes:
        for note in notes:
            lines.append(f"- {note}")
    else:
        lines.append("- Follow Master file for all unspecified rules")
    lines.append("")
    
    # Usage at build time
    lines.append("---")
    lines.append("")
    lines.append("## Build Time Instructions")
    lines.append("")
    lines.append("1. Read this file first for page-specific rules")
    lines.append("2. Fall back to MASTER.md for unspecified rules")
    lines.append("3. Apply all overrides before Master defaults")
    lines.append("4. Ensure no conflicts between override and Master rules")
    lines.append("")
...
def _generate_intelligent_overrides(page_name: str, page_query: str, design_system: dict) -> dict:
    """
    Generate intelligent page-specific overrides based on page name and query analysis.
    
    Uses a rule-based system to detect page types and generate meaningful overrides
    instead of generic empty structures.
    """
    page_lower = page_name.lower()
    query_lower = (page_query or "").lower()
    
    # Default overrides (no page-specific knowledge)
    default_overrides = {
        "page_type": "General",
        "layout": {},
        "spacing": {},
        ...
        "components": [],
        "unique_components": [],
        "notes": ["This is a general page — follow Master file for specifications."]
    }
    ...
    # Dashboard page
    if any(kw in page_lower or kw in query_lower for kw in ["dashboard", "analytics", "metrics", "overview", "stats"]):
        overrides["page_type"] = "Dashboard / Analytics"
        overrides["layout"] = {
            "Structure": "Sidebar + Main content area with widget grid",
            "Grid": "12-column responsive grid for widget placement",
            "Sidebar": "Collapsible, 240px expanded, 64px collapsed",
            "Header": "Sticky top bar with search, notifications, user menu",
            "Widget Layout": "2-col on tablet, 3-col on desktop, 1-col on mobile"
        }
        overrides["spacing"] = {
            "Widget Padding": "16px (less than Master default for data density)",
            "Grid Gap": "16px between widgets",
            "Sidebar Padding": "16px horizontal, 8px vertical"
        }
        overrides["components"] = [
            "Data cards with KPI indicators",
            "Charts with time-range selectors",
            "Data tables with sorting/filtering",
            "Status badges and notification toasts",
            "Search bar with autocomplete"
        ]
        overrides["unique_components"] = [
            "Interactive chart widgets (line/bar/pie combo)",
            "KPI metric cards with sparklines",
            "Data grid with inline editing",
            "Filter bar with date range picker"
        ]
        overrides["notes"] = [
            "Dashboard pages prioritize data density over whitespace",
            "Ensure charts have proper empty states",
            "Use skeletons for loading states, not spinners",
            "Responsive: stack widgets vertically on mobile"
        ]
    
    # Landing page
    elif any(kw in page_lower or kw in query_lower for kw in ["landing", "home", "hero", "landing page"]):
        overrides["page_type"] = "Landing / Marketing"
        overrides["layout"] = {
            "Hero": "Full-width with gradient background, central CTA",
            "Content Flow": "Hero → Features → Testimonials → Pricing → CTA → Footer",
            "Max Width": "Full-bleed hero, 1280px max-width for content sections",
            "Sticky Elements": "Floating navbar with background blur on scroll"
        }
        overrides["spacing"] = {
            "Hero Padding": "80-120px vertical (more spacious than Master)",
            "Section Gap": "64px between sections",
            "Content Padding": "32px on desktop, 16px on mobile"
        }
        overrides["typography"] = {
            "Hero Heading": "48-64px, bold, tighter letter-spacing",
            "Subtitle": "20-24px, regular weight, max-width 600px"
        }
        overrides["components"] = [
            "Hero section with primary CTA button",
            "Feature grid with icons (3-col, 2-col, 1-col responsive)",
            "Testimonial carousel or cards",
            "Pricing cards (3 tiers recommended)",
            "Final CTA section (bottom of page)"
        ]
        overrides["unique_components"] = [
            "Hero section with animated background",
            "Feature comparison table or grid",
            "Testimonial carousel with avatars",
            "Pricing toggle (monthly/yearly)"
        ]
        overrides["notes"] = [
            "Landing pages should be highly visual with generous whitespace",
            "Above-fold content must clearly communicate value proposition",
            "CTA buttons should use accent color and be visually prominent",
            "Social proof (logos, testimonials) should appear early",
            "Mobile: stack all sections vertically, reduce hero text size"
        ]
    
    # Auth page (login/signup)
    elif any(kw in page_lower for kw in ["login", "signup", "register", "auth", "sign-in", "sign-up", "log-in"]):
        overrides["page_type"] = "Authentication"
        overrides["layout"] = {
            "Structure": "Centered card layout, split-screen optional",
            "Form Width": "400px max-width, centered",
            "Split Screen": "50/50 with illustrative/brand content on left (desktop only)"
        }
        overrides["spacing"] = {
            "Form Padding": "32px inside card",
            "Vertical Spacing": "24px between form fields",
            "Card Margin": "24px from edges on mobile"
        }
        overrides["components"] = [
        ...
        overrides["unique_components"] = [
            "OAuth provider buttons with brand colors",
            "Password strength indicator",
            "Magic link / OTP input field"
        ]
        overrides["notes"] = [
            "Auth pages should be minimal and distraction-free",
            "Remove header/footer navigation to focus on conversion",
            "Show clear error states for validation failures",
            "Loading state on submit button to prevent double-clicks",
            "Password fields should have show/hide toggle"
        ]
    
    # Settings page
    elif any(kw in page_lower for kw in ["settings", "preferences", "profile", "account"]):
        overrides["page_type"] = "Settings / Profile"
        overrides["layout"] = {
            "Structure": "Sidebar navigation + detail panel",
            "Navigation": "Vertical tab list (200px) on desktop, horizontal tabs on mobile",
            "Content": "Single panel that changes based on selected setting"
        }
        overrides["spacing"] = {
            "Section Padding": "24px between setting groups",
            "Form Group Spacing": "16px between related fields"
        }
        overrides["components"] = [
        ...
        overrides["notes"] = [
            "Settings should not auto-save (require explicit save action)",
            "Show unsaved changes indicator",
            "Group related settings under clear section headers",
            "Include undo/revert capability where possible"
        ]
